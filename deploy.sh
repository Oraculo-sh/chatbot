...
# ==============================================================================
# Função auxiliar: set_env_var
# Atualiza uma variável no arquivo .env via sed. Se não existir, adiciona.
    ...
    while true; do
        ...
        while true; do
            ...
            if [[ -z "$DOMAIN" ]]; then
                ...
                echo -e "${RED}Formato inválido. Use um domínio como: example.com ou sub.example.com.br${NC}"
            else
                break
            fi
        done
...
inject_service_properties() {
    ...
    for srv in $services; do
        ...
        if [ -n "$block" ]; then
            local tmpfile
            tmpfile=$(mktemp "${DEPLOY_DIR}/tmp.XXXXXX.yml")
            # Encontra o final do bloco do serviço no compose e insere as novas properties antes do próximo serviço
            awk -v s="  $srv:" -v b="$block" '
                BEGIN { found=0 }
                $0 == s { found=1; print; next }
                found && /^  [a-zA-Z0-9_-]+:/ { print b; found=0; print; next }
                found && /^[a-zA-Z]/ { print b; found=0; print; next }
                { print }
                END { if (found) print b }
            ' docker-compose.yml > "$tmpfile" && mv "$tmpfile" docker-compose.yml
        fi
    done
}
...
pull_images() {
    ...
    docker compose pull n8n postgres-chatbot redis-chatbot minio-chatbot
    ...
    docker compose pull typebot-builder typebot-viewer

    log "-> Lote 4/5 [Data & APIs]: Evolution, Redis e MinIO (Serviços Leves)..."
    docker compose pull evolution-api evolution-frontend
